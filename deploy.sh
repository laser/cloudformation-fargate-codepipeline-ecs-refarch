...
IMAGE_TAG="166875342547.dkr.ecr.us-east-1.amazonaws.com/prehistorical:latest"

SERVICE_ARN=$(
    aws cloudformation \
        describe-stacks --query 'Stacks[0].Outputs[?OutputKey==`HelloworldServiceArn`].OutputValue' \
        --stack-name $ENV_NAME_ARG | jq '.[0]' | sed -e "s;\";;g")

STACK_NAME=$(
    aws cloudformation \
        describe-stacks --query 'Stacks[0].Outputs[?OutputKey==`HelloworldServiceStackName`].OutputValue' \
        --stack-name $ENV_NAME_ARG | jq '.[0]' | sed -e "s;\";;g")
